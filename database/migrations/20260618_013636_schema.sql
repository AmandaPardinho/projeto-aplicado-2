...
\restrict vTKdMBAxwjJ7ZpmyywK5cLkStdhkIcotS61kSdoJlR64wnLFGDVUVYvxglYHohy
...
CREATE TABLE public.client (
    ...
    CONSTRAINT client_name_format_check CHECK (((name ~ '^[A-Za-zÀ-ÖØ-öø-ÿ ''-]+$'::text) AND (char_length(name) >= 3)))
);
...
CREATE TABLE public.instructor (
    ...
    CONSTRAINT instructor_name_format_check CHECK (((name ~ '^[A-Za-zÀ-ÖØ-öø-ÿ ''-]+$'::text) AND (char_length(name) >= 3)))
);
...
CREATE TABLE public.plan (
    ...
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT plan_monthly_fee_check CHECK (((monthly_fee >= (0)::numeric) AND (monthly_fee <= (100000)::numeric))),
    CONSTRAINT plan_name_not_blank_check CHECK ((char_length(btrim(name)) >= 2)),
    CONSTRAINT plan_sessions_per_month_check CHECK (((sessions_per_month > 0) AND (sessions_per_month <= 31)))
);
...
\unrestrict vTKdMBAxwjJ7ZpmyywK5cLkStdhkIcotS61kSdoJlR64wnLFGDVUVYvxglYHohy
